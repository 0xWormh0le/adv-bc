...
describe("Dividends", function() {
    ...
    it('name', async function() {
        expect(
            await dividends.name()
        ).to.eq('stkBribeAavePool')
    })

    it('symbol', async function() {
        expect(
            await dividends.symbol()
        ).to.eq('stkBrAave')
    })
    ...
    it('stake', async function() {
        ...
        expect(userData.map(x => x.toNumber())).to.deep.eq([0, 0])
    ...
    it('rescueFunds', async function() {
        ...
        await expect(
            dividends.connect(user2).rescueFunds(testVars.usdc.address)
        ).to.be.revertedWith('Ownable: caller is not the owner')
        ...
        await expect(
            dividends.rescueFunds(testVars.usdc.address)
    ...
    it('transfer', async function() {
        ...
        await dividends.accrueDividend()
        // transfer balance to peter now
        await dividends.transfer(
            peter.address,
            amountToDeposit,
        )
        ...
        expect((await dividends.dividendOf(bob.address)).toNumber()).to.eq(amountToDeposit)
        expect((await dividends.dividendOf(peter.address)).toNumber()).to.eq(amountToDeposit)
