...
import {
  ...
  latestTime,
  setupTest,
  setUserAaveTokenBalance,
  setUserStkAaveTokenBalance,
} from "./lib";
...
async function userPoolDeposit(testVars: ITestVars, user: SignerWithAddress) {
  await setUserAaveTokenBalance(testVars, [user], amountToDeposit);

  await setUserStkAaveTokenBalance(testVars, [user], amountToDeposit);
  ...
  await testVars.aavePool
    .connect(user)
    .deposit(testVars.aaveTokenV2.address, user.address, amountToDeposit, true);
  await testVars.aavePool
    .connect(user)
    .deposit(testVars.stkAave.address, user.address, amountToDeposit, true);
}
...
async function userBid(
  ...
  await testVars.usdc
    .connect(user)
    .approve(testVars.aavePool.address, amountToApprove || amountToBid * 5);
  await testVars.aavePool.connect(user).bid(user.address, testVars.proposalId, amountToBid, true);
}
...
describe("AavePool", function () {
  ...
  it("deposit", async function () {
    ...
    // set user
    await setUserAaveTokenBalance(testVars, [bob, peter], amountToDeposit);
    await setUserStkAaveTokenBalance(testVars, [bob, peter], amountToDeposit);

    await Promise.all(
      [bob, peter].map(async (account) => {
        ...
        await expect(
          aavePool
            .connect(account)
            .deposit(aaveTokenV2.address, account.address, amountToDeposit, true)
        )
          .to.emit(aavePool, "Deposit")
          .withArgs(aaveTokenV2.address, account.address, amountToDeposit, await latestTime());

        // stk aave deposit
        await expect(
          aavePool.connect(account).deposit(stkAave.address, account.address, amountToDeposit, true)
        ).to.emit(aavePool, "Deposit");
        // .withArgs(
        //   stkAave.address,
        //   account.address,
  ...
  it("withdraw", async () => {
    ...
    await expect(
      aavePool.connect(bob).withdraw(aaveTokenV2.address, bob.address, 0, true)
    ).to.revertedWith("INVALID_AMOUNT");

    // cannot withdraw more than deposited
    await expect(
      aavePool.connect(bob).withdraw(aaveTokenV2.address, bob.address, amountToDeposit * 3, true)
    ).to.revertedWith("INVALID_BALANCE");

    // successfully withdraws
    await expect(
      aavePool.connect(bob).withdraw(aaveTokenV2.address, bob.address, amountToDeposit, true)
    ).to.emit(aavePool, "Withdraw");

    await expect(
      aavePool.connect(bob).withdraw(stkAave.address, bob.address, amountToDeposit, true)
    ).to.emit(aavePool, "Withdraw");
  ...
  it("Bid - succeeds", async function () {
    ...
    await expect(withPeter.bid(peter.address, proposalId, amountToBid, true))
    ...
    await expect(withPeter.bid(peter.address, proposalId, amountToBid * 2, true))
      .to.emit(aavePool, "HighestBidIncreased")
      .withArgs(proposalId, peter.address, peter.address, peter.address, amountToBid * 3, true);

    await expect(aavePool.bid(bob.address, proposalId, amountToBid * 4, true))
      .to.emit(aavePool, "HighestBidIncreased")
      .withArgs(proposalId, peter.address, bob.address, bob.address, amountToBid * 4, true);
  });
  ...
  it("bid fails after end time", async function () {
    ...
    await expect(
      aavePool.connect(peter).bid(peter.address, testVars.proposalId, amountToBid, true)
  ...
  it("vote: succeeds", async function () {
    ...
    await expect(aavePool.connect(peter).vote(testVars.proposalId)).to.emit(aavePool, "Vote");
  });
  ...
  it("setStartTimestamp", async function () {
    ...
    await expect(aavePool.setStartTimestamp(latestTimes, 1)).to.emit(
      aavePool,
      "SetBribeRewardStartTimestamp"
    );
    // .withArgs(period.add(1), await );

    await expect(aavePool.setStartTimestamp(latestTimes - 500, 1)).to.be.revertedWith(
      "INVALID_START_TIMESTAMP"
    );

    expect((await aavePool.bribeRewardConfig()).startTimestamp).to.equal(latestTimes);
  });
  ...
  it("setEndTimestamp", async function () {
    ...
    await expect(aavePool.setEndTimestamp(latestTimes)).to.emit(
      aavePool,
      "SetBribeRewardEndTimestamp"
    );
    // .withArgs(period.add(1), await time.latest());

    await expect(aavePool.setEndTimestamp((await latestTime()) - 500)).to.be.revertedWith(
      "INVALID_END_TIMESTAMP"
    );

    await expect(aavePool.setStartTimestamp(latestTimes, 10)).to.be.revertedWith("HIGH_TIMESTAMP");

    expect((await aavePool.bribeRewardConfig()).endTimestamp).to.equal(latestTimes);
  });
  ...
  it("setFeeDistributor", async function () {
    ...
    await expect(aavePool.connect(user2).setFeeRecipient(user2.address)).to.be.revertedWith(
      "Ownable: caller is not the owner"
    );

    await expect(aavePool.setFeeRecipient(ethers.constants.AddressZero)).to.be.revertedWith(
      "INVALID_RECIPIENT"
    );

    await expect(aavePool.setFeeRecipient(user2.address))
  ...
  it("setRewardPerSecond", async function () {
    ...
    await expect(aavePool.setRewardPerSecond(10)).to.emit(aavePool, "SetBribeRewardPerSecond");
    // .withArgs(period.add(1), await time.latest());

    expect((await aavePool.bribeRewardConfig()).rewardAmountDistributedPerSecond).to.equal(10);
  });
  ...
  it("refund: revert if refund active proposal", async function () {
    ...
    await expect(aavePool.connect(bob).refund(proposalId)).to.revertedWith("PROPOSAL_ACTIVE");
  });
  ...
  it("refund: not working for no bid", async function () {
    ...
    await expect(aavePool.connect(bob).refund(proposalId)).to.not.emit(aavePool, "Refund");
  });
  ...
  it("refund: success cancelled proposal", async function () {
    ...
    await expect(aavePool.connect(peter).bid(peter.address, proposalId, 50, true)).to.revertedWith(
    ...
    expect(await usdc.balanceOf(peter.address)).to.equal(prevBalance.add(amountToBid));
  });
  ...
  it("blockProposalId", async function () {
    ...
    await expect(aavePool.blockProposalId(proposalId)).to.emit(aavePool, "BlockProposalId");
    // .withArgs(this.proposalId, currentTime);

    await expect(aavePool.blockProposalId(proposalId)).to.revertedWith("PROPOSAL_INACTIVE");

    expect(await usdc.balanceOf(peter.address)).to.equal(prevBalance.add(amountToBid));

    await expect(aavePool.connect(peter).bid(peter.address, proposalId, 10, true)).to.revertedWith(
      "PROPOSAL_BLOCKED"
    );
  });
  ...
  it("unblockProposalId", async function () {
    ...
    await expect(aavePool.unblockProposalId(proposalId)).to.emit(aavePool, "UnblockProposalId");
    // .withArgs(this.proposalId, currentTime);

    await expect(aavePool.unblockProposalId(proposalId)).to.revertedWith("PROPOSAL_ACTIVE");
  ...
  it("multicall", async function () {
    ...
    await setUserAaveTokenBalance(testVars, [bob], amountToDeposit);

    await setUserStkAaveTokenBalance(testVars, [bob], amountToDeposit);
    ...
    await aavePool
      ...
      .multicall([
        ...
        aavePool.interface.encodeFunctionData("deposit", [stkAave.address, bob.address, 10, true]),
        aavePool.interface.encodeFunctionData("bid", [bob.address, proposalId, 10, true]),
      ]);

    expect(await (await aavePool.bids(proposalId)).highestBid.toNumber()).to.equal(10);
    expect(await (await aavePool.bids(proposalId)).highestBidder).to.equal(bob.address);
  ...
  it("pause", async function () {
    ...
    await expect(aavePool.connect(peter).pause()).to.be.revertedWith(
      "Ownable: caller is not the owner"
    );

    await expect(aavePool.pause()).to.emit(aavePool, "Paused").withArgs(bob.address);

    await expect(aavePool.deposit(aaveTokenV2.address, bob.address, 1, true)).to.be.revertedWith(
      "Pausable: paused"
    );
    ...
    await expect(aavePool.bid(peter.address, proposalId, 1, true)).to.be.revertedWith(
      "Pausable: paused"
    );
  });
  ...
  it("unpause", async function () {
    ...
    await expect(aavePool.connect(peter).unpause()).to.be.revertedWith(
      "Ownable: caller is not the owner"
    );

    await aavePool.pause();

    await expect(aavePool.unpause()).to.emit(aavePool, "Unpaused").withArgs(bob.address);
  });
});
